...
- **Semantic Embeddings** — Starts with an offline deterministic baseline, then optionally upgrades to local Transformers.js embeddings in a Web Worker using WebGPU/WASM with automatic fallback.
...
| Graph / Vector Search | [PolyGraph](https://github.com/anomalyco/polypack) — persistent graph with OPFS adapter, vector similarity search, and adaptive memory |
| Local Embeddings | [Transformers.js](https://huggingface.co/docs/transformers.js) — optional browser sentence embeddings with FeatureHashEmbedding fallback |
...
- **Embedding upgrade path** — FeatureHashEmbedding is available immediately and offline. On capable browsers, a local `all-MiniLM-L6-v2` Transformers.js model is loaded in a Worker, preferring WebGPU and falling back to WASM. Existing persisted vectors are rebuilt in background batches and tagged with provider/version and input-hash metadata.
